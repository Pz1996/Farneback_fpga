...
int main() {
	Mat pre_rgb, aft_rgb, pre, aft, pre_rgb2;
#ifdef NOCAMERA
	pre_rgb = imread("pre.jpg", IMREAD_COLOR);
	aft_rgb = imread("aft.jpg", IMREAD_COLOR);
#else
	VideoCapture cap(0);
	if (!cap.isOpened())
		return -1;
...
//		pre_rgb.copyTo(aft_rgb);
		...
		Mat flow;
		calcOpticalFlowFarneback(pre, aft, flow, 0.5, 1, 11, 3, 7, 1.5, 0);
